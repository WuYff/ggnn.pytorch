...
class GGNN(nn.Module):
    ...
    def forward(self, prop_state, annotation, A):
        ...
        #     for n in range(len(output[b])):
        #         if output[b][n] < 0.5:
        #             output[b][n] = 0
        #         else:
        #             output[b][n] = 1
        ## RuntimeError: one of the variables needed for gradient computation has been modified by an inplace operation: [torch.cuda.DoubleTensor [10, 40, 40]], which is output 0 of TanhBackward, is at version 16000; expected version 0 instead. Hint: enable anomaly detection to find the operation that failed to compute its gradient, with torch.autograd.set_detect_anomaly(True).
        return output
